...
if(isset($_GET["roomid"])){
    $roomid=$_GET["roomid"];
    $result=$web->whichRoom($roomid);
    $row=mysqli_fetch_row($result);
}
if(isset($_POST["start"]) && isset($_POST["end"]) && isset($_POST["announce"]) && isset($_POST["date"]) && isset($_POST["roomid"])){
    if($web->timeCheck($_POST["date"],$_POST["start"],$_POST["end"])){
        if($web->roomNum($_POST["roomid"])>0){
            $amount=$web->amountCal($_POST["roomid"],$_POST["start"],$_POST["end"],$_POST["announce"]);
            if($web->recordInsert($userid,$_POST["date"],$_POST["start"],$_POST["end"],$_POST["roomid"],$_POST["announce"],$amount) && $web->roomReduce($_POST["roomid"])){
                echo "<script> {window.alert('訂房成功');location.href='reservecancel.php'} </script>";
            }
            else{
                echo "<script> {window.alert('訂房失敗');location.href='reserve.php'} </script>";
            }
        }
        else{
            echo "<script> {window.alert('房間數量不足');location.href='reserve.php'} </script>";
        }
    }
    else{
        echo "<script> {window.alert('時間選取錯誤');location.href='reserve.php'} </script>";
    }
}
?>
<!DOCTYPE html>
